...
//FOR GUI PURPOSES ONLY FROM HERE
...
function GenerateLegalMoves() {
	...
	for(index = GameBoard.moveListStart[GameBoard.ply]; index < GameBoard.moveListStart[GameBoard.ply+1]; ++index) {
        ...
        if(MakeMove(move)){
            GameBoard.legalMoveList.push(move);
            TakeMove();
...
function GenerateLegalMovesSq(sq) {
    var index;
	var move;
	//console.log('MoveList:');
    legalMoves = [];
	...
	for(index = 0; index < GameBoard.legalMoveList.length; ++index) {
        ...
        var MvStr = FileChar[ff] + RankChar[rf];
        if(MvStr == sq){
            var ft = FilesBrd[TOSQ(move)];
            var rt = RanksBrd[TOSQ(move)];
            var toStr = FileChar[ft] + RankChar[rt];
            legalMoves.push(toStr);
        }
	}
